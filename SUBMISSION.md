...
> LLM extraction fails silently: one model returns a confident wrong value and you never know. Crosscheck sends the same extraction prompt to two providers in parallel through the BTL gateway. Fields where both providers agree are auto-accepted; fields where they disagree are adjudicated by a judge pass and flagged for human review — disagreement is the confidence signal. If a provider returns a 5xx, drops the connection, or times out (all of which the gateway does intermittently — we hit a live connection drop during testing), the call automatically fails over to the other provider, so it degrades gracefully instead of crashing. On a labeled extraction benchmark, Crosscheck's accuracy is on par with the stronger single model (~93%) — the honest finding is that two capable models mostly agree, so cross-checking doesn't boost raw accuracy on clean text. Its measured value is a calibrated per-field confidence signal (100% flag precision, only ~3% of fields need review) plus cross-provider resilience. When both providers share the same bias, cross-checking is blind — reported honestly, not hidden. All of this is only possible because the BTL runtime is a multi-provider gateway.
...
> "Here's the honest result. Two strong models mostly agree, so accuracy is about the same whether you use one model or Crosscheck — around 93%. What Crosscheck adds is a confidence signal: every field it flags for review is a genuine cross-provider discrepancy — 100% precision — and only about 3% of fields need a human. And I report the blind spot honestly: when both providers share the same bias, cross-checking can't see it. It's a safety net and a resilience layer, not a magic accuracy boost."
